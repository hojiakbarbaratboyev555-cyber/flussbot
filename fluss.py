...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fluss:app", host="0.0.0.0", port=8000)        await message.answer(
            "Salom! Siz botdan foydalanishingiz mumkin.\n"
            "Murojaatingizni yuborish uchun /murojaat buyrug'ini ishlating."
        )
    else:
...
async def handle_murojaat(message: types.Message):
    ...
    # Murojaat matni /murojaat so'zini olib tashlab qoldiramiz
    text_to_send = (
        f"📌 Murojaat:\n"
        f"👤 Ism: {full_name}\n"
        f"🆔 ID: {user.id}\n"
        f"🔗 Username: {username}\n\n"
        f"✉️ Murojaat matni: {message.text.replace('/murojaat','').strip() or 'Yo\'q matn'}"
    )
...
# --- Render yoki boshqa server porti bilan ishga tushirish ---
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fluss:app", host="0.0.0.0", port=8000, reload=True)        await message.answer(
            "Salom! Siz botdan foydalanishingiz mumkin.\n"
            "Murojaatingizni yuborish uchun /murojaat buyrug‘ini ishlating."
        )
    else:
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fluss:app", host="0.0.0.0", port=8000, reload=True)            f"⚠️ Botdan foydalanish uchun avval kanalimizga obuna bo‘ling: {CHANNEL_USERNAME}\n"
            "Obuna bo‘lgach /start buyrug‘ini qayta yuboring."
        )
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("bot:app", host="0.0.0.0", port=8000, reload=True)        await message.answer(
            f"⚠️ Botdan foydalanish uchun avval kanalimizga obuna bo‘ling: {CHANNEL_USERNAME}\n\n"
            "Obuna bo‘lgach /start buyrug‘ini qayta yuboring."
        )
